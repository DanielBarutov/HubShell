...
| F-16 | fix | P0 | session/portal/win-client | поминутный остаток берётся из ставки текущей сессии и совпадает в серверном снимке и Windows-клиенте |
...
22. [x] **F-16.** [implementation complete: server snapshot, portal zone filter
    и Win Client projection покрыты; live polling остаётся] В снимке активной поминутной сессии рассчитывать остаток
    только по ставке её активного тарифа. Клиент Windows показывает это же
    значение, а не ноль или ставку другой зоны.
