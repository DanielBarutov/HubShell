  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      <div className="content-wrap">
        ...
        {ui.section === "bookings" && <BookingsView api={LIVE_MODE ? api : undefined} pcs={currentPcs} clients={currentClients} zoneOptions={zoneOptions} onNewBooking={() => dispatch(openBooking(undefined))} onEditBooking={(reservation) => dispatch(openBookingEdit(reservation))} refreshKey={ui.bookingRefreshKey} />}
        ...
        {ui.section === "catalog" && <CatalogView api={LIVE_MODE ? api : undefined} groups={workspace.groups} refreshKey={ui.catalogRefreshKey} onNewTariff={LIVE_MODE ? () => dispatch(openPanel("tariff")) : undefined} onNewProduct={LIVE_MODE ? () => { dispatch(selectProduct(null)); dispatch(openPanel("product")); } : undefined} onEditProduct={LIVE_MODE ? (product) => { dispatch(selectProduct(product)); } : undefined} onSellProduct={LIVE_MODE ? (product) => openSalePanel(undefined, product) : undefined} onNewDiscount={LIVE_MODE ? () => dispatch(openPanel("discount")) : undefined} />}
        {ui.section === "analytics" && <AnalyticsView api={LIVE_MODE ? api : undefined} clients={currentClients} onClient={openClientPanel} />}
        {ui.section === "cash" && <CashView api={LIVE_MODE ? api : undefined} shifts={workspace.cashShifts} onOpenShift={LIVE_MODE ? () => dispatch(openPanel("cash-open")) : undefined} onRecordMovement={LIVE_MODE ? (shift) => dispatch(openCashMovement(shift)) : undefined} onCloseShift={LIVE_MODE ? (shift) => dispatch(openCashClose(shift)) : undefined} />}
        {ui.section === "settings" && <SettingsView api={LIVE_MODE ? api : undefined} pcs={currentPcs} refreshKey={ui.settingsRefreshKey} onNewGroup={LIVE_MODE ? () => dispatch(selectGroup(null)) : undefined} onEditGroup={LIVE_MODE ? (group) => dispatch(selectGroup(group)) : undefined} onNewPaymentMethod={LIVE_MODE ? () => dispatch(selectPaymentMethod(null)) : undefined} onEditPaymentMethod={LIVE_MODE ? (method) => dispatch(selectPaymentMethod(method)) : undefined} />}
      </div>
    </main>
    <PanelHost />
  </div>;
