...
| Plan 44 phase 0/early F-01/F-02/F-03 | контракты для client groups/debt policy, payment-method independence, package fallback, stop acknowledgement, server-anchored timer и notification boundaries; regression matrix F-01…F-13; positive-balance fallback, Core stop-order и countdown projection | contracts, owner plans и matrix обновлены; targeted backend slice `44 passed`, Ruff clean; Win Core `38 passed`, включая projection и stop-order tests; frontend `19 passed`, proto layout `19 passed` | F-01b/group debt, F-02 Windows visual timer и F-03 full access-gate/restart всё ещё требуют DSN/live Compose и Windows native evidence |
