...
| --- | --- | --- | --- |
| Backend foundation и модули | Ruff, unit/API/contract tests, metered billing, package windows/auto-next, locked consumption delta, snapshot, transfer, offline replay, settlement review, manager credential, lockdown policy, analytics, payment parts, guest paid-start и entry decision | `128 passed, 15 skipped` без DSN; `143 passed` с dev PostgreSQL/Redis DSN (2026-09-02) | PostgreSQL package locked-delta, transfer idempotency/concurrency и offline duplicate-debit checks добавлены; fault injection и полная transaction matrix остаются |
