...
| F-14 | fix | P1 | clients/entitlements | история списания за пакет показывает название тарифа, а не технический идентификатор пакета |
| F-15 | fix | P2 | frontend/cash | селектор времени автооткрытия и автозакрытия не меняет флажок правила и сохраняет выбранное значение |
| F-16 | fix | P0 | session/portal/win-client | поминутный остаток берётся только из действующей ставки текущей зоны; пакетные тарифы в расчёте не участвуют |
| F-17 | fix | P0 | entitlements | недоступный сейчас пакет не блокирует подходящий пакет, следующий в очереди |
| F-18 | fix | P1 | frontend/map | карточка карты не включает в окончание минуты пакета, который ещё нельзя расходовать |
| F-19 | ui fix | P2 | win-client | подтверждения переноса и покупки тарифа компактны, используют стили приложения и не оставляют пустой отступ |
| F-20 | feature | P2 | frontend/map | на карте есть отдельное быстрое действие «Продать товар», открывающее товары и напитки |
| F-21 | ui fix | P2 | frontend/sales | карточки товаров и напитков в продаже плотнее карточек тарифов и помещают семь позиций на широком экране |
| F-22 | fix | P1 | backend/portal | тариф можно купить в долг в пределах лимита; пакет сверх лимита не показывается клиенту как пакет в очереди |
...
5. [x] **F-01.** [implementation complete: positive-balance fallback, bounded group debt и stop при исчерпании реализованы и покрыты; отдельные проверки проходят пять бесплатных и три пакетные минуты по одной, а также остановку обычного клиента ровно на пятой минуте без денег или ставки зоны. Фоновая задача списания загружает правила группы, поэтому должник не выходит на пятой минуте и получает первое списание на шестой; добавлена проверка с настоящей PostgreSQL для сохранённого лимита долга. Исправлена промежуточная проверка: теперь сравнивается только новая сумма списания, а не вся накопленная стоимость сессии; добавлены тесты длинной сессии, двух последовательных сессий, точной границы и подробного отказа. Команды `session.stop`/`display.lock` проверены; PostgreSQL-проверку и ручную проверку после развёртывания нужно выполнить отдельно]
...
6. [x] **F-02.** [исправлено: одинаковый ответ сервера не
   перезапускает локальный отсчёт; тест проходит 5 бесплатных и 3 пакетные
   минуты, а карта не сдвигает окончание от запоздалого снимка; ручная проверка
   на Windows остаётся]
...
### Фаза 6 — дополнения из `fixes.md`

20. [x] **F-14.** [implementation complete: новые записи и миграция старых
    записей используют название тарифа; PostgreSQL migration smoke остаётся]
    Сохранять в причине списания название купленного тарифа.
    В карточке клиента вместо `Package purchase <id>` показывать, например,
    `Покупка тарифа «Ночной тест»`.
21. [x] **F-15.** [implementation complete: разбор значения времени и
    component regression реализованы; browser smoke остаётся] Развести флажок автоматического открытия смены и селектор
    времени в разметке. Выбор часов и минут не должен переключать правило.
22. [x] **F-16.** [implementation complete: portal исключает пакетные тарифы
из расчёта остатка и берёт только действующую поминутную ставку зоны; regression
покрывает пример 150 ₽, 1 ₽/мин и пакет 3 мин за 50 ₽; при запуске backend
восстанавливает внутреннюю ставку для старой зоны, поэтому баланс 100 ₽ снова
даёт минуты, а после бесплатных 5 минут начинается поминутное списание; live
проверка после перезапуска backend остаётся]
В снимке активной поминутной сессии рассчитывать остаток только по ставке её
...
24. [x] **F-18.** [implementation complete: карта отделяет недоступный пакет от
    расчёта времени и component regression реализована] В карте суммировать для текущего окончания только доступные
    сейчас минуты. Недоступный ночной пакет остаётся показанным в очереди с
    понятным статусом, но не удлиняет окончание сессии до окна расходования.
25. [x] **F-19/F-20.** [implementation complete: компактные окна подтверждения
    в Windows-клиенте и отдельное действие карты с component regression]
    Оформить подтверждения переноса и покупки тарифа стилями приложения и
    добавить отдельную быструю продажу товара с карты.
26. [x] **F-21.** [implementation complete: отдельная адаптивная сетка товаров
    на 7/5/4/2 колонки; frontend build и SaleWorkspace tests проходят]
    Сделать карточки товаров и напитков компактнее карточек тарифов, не меняя
    плотность выбора игрового времени.
27. [x] **F-22.** [implementation complete: неудачная покупка тарифа с
    депозитного баланса сверх лимита долга не попадает в пользовательскую
    очередь; regression покрывает обычную покупку, покупку в пределах и за
    пределами лимита, а также ответ Windows-клиента на границе пяти минут]
    Разрешить покупку тарифа в долг в пределах лимита группы. Не показывать
    клиенту неоплаченный пакет сверх лимита как «В очереди» и не возвращать
    успешный результат для повторной попытки с тем же ключом операции.
