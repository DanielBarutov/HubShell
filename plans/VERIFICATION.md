...
| --- | --- | --- | --- |
| Backend foundation и модули | Ruff, unit/API/contract tests, metered billing, package windows/auto-next, locked consumption delta, snapshot, transfer, offline replay, settlement review, manager credential, lockdown policy, analytics, payment parts, guest paid-start и entry decision | `128 passed, 13 skipped` без DSN; `141 passed` с dev PostgreSQL/Redis DSN (2026-09-02) | Полная package/debit/transfer/offline transaction matrix и fault injection остаются отдельной проверкой; package locked-delta check уже добавлен |
