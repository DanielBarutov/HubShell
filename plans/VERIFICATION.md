...
| gRPC | generated protobuf, client portal entitlement queue/explicit activation, analytics/workstation/business registration, auth/audit и optional TLS policy; внешний `SystemService/GetHealth` и авторизованный `SessionService/GetSnapshot` через `:51051` | успешно в текущем Compose smoke, `gameclub-backend:ok:0.1.0`, snapshot schema `1` | external TLS deployment и full gRPC flow matrix не проверялись; private-LAN insecure transport допускается |
