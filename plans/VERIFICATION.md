...
| --- | --- | --- | --- |
| Backend foundation и модули | Ruff, unit/API/contract tests, metered billing, package windows/auto-next, locked consumption delta, snapshot, transfer, offline replay, settlement review, manager credential, lockdown policy, analytics, payment parts, guest paid-start и entry decision | `130 passed, 16 skipped` без DSN; `146 passed` с dev PostgreSQL/Redis DSN (2026-09-02) | PostgreSQL package locked-delta, transfer idempotency/concurrency, offline duplicate-debit и sale payload-conflict checks добавлены; fault injection и полная transaction matrix остаются |
...
| Product contract audit | сравнение backend/frontend/win-client product contracts с кодом, CODEX и планами; implementation progress зафиксирован в плане 29 | source/unit + текущий Compose/headed slice реализован; PostgreSQL DSN suite добавила package/transfer/offline concurrency evidence | fault injection, native Windows/kiosk и production security evidence остаются незакрыты |
