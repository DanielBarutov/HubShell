...
| F-01c | Длинная поминутная сессия получает очередной тик после частичного списания; затем клиент запускает вторую сессию | проверяется только новая дельта; накопленная стоимость не воспринимается как новая задолженность; после 151 и 75 минут сессии не закрываются при исходном балансе 500 ₽ | backend application test с фиксированными моментами, затем PostgreSQL test | добавлены `test_per_minute_meter_checks_only_the_next_delta_mid_session`, `test_two_long_sessions_do_not_stop_after_successful_partial_debits` и проверка завершения только при следующей недоступной минуте; PostgreSQL и ручная проверка после развёртывания остаются |
