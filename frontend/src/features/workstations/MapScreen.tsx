...
export function MapView({ onPc, onSalePc, onSellProduct, onBookPc, onEditPc, pcs, group, setGroup, zoneOptions, onNewWorkstation, onPositionsChange }: {
  ...
  onSalePc: (pc: Workstation) => void;
  onSellProduct?: () => void;
  onBookPc: (workstationId?: string) => void;
  onEditPc: (pc: Workstation) => void;
  pcs: Workstation[];
  group: string;
  setGroup: (group: string) => void;
  zoneOptions: string[];
  onNewWorkstation?: () => void;
  onPositionsChange?: (changes: Array<{ workstationId: string; position: number }>) => Promise<void>;
}) {
  ...
  const handleDrop = async (event: React.DragEvent<HTMLDivElement>, targetSlot: number) => {
    ...
    if (target) changes.push({ workstationId: target.pc.id, position: source.slot + 1 });
  ...
  return <>
    <div className="page-heading"><div><p className="eyebrow">Оборудование · Рабочая карта</p><h1>Карта клуба</h1><p className="subheading">{editMode ? "Перетащите места в нужные ячейки и сохраните планировку." : "Нажмите на свободное место, чтобы оформить продажу; управление ПК — через кнопку ⋯."}</p></div><div className="heading-actions"><button className={`secondary-button map-edit-toggle ${editMode ? "active" : ""}`} aria-pressed={editMode} onClick={() => { setEditMode((value) => !value); setContextPcId(null); }}><Edit3 size={15} /> {editMode ? "Завершить редактирование" : "Редактировать карту"}</button><button className="secondary-button" onClick={onSellProduct}><ShoppingCart size={16} /> Продать товар</button><button className="primary-button" disabled={!onNewWorkstation} onClick={onNewWorkstation}><Plus size={17} /> Добавить место</button></div></div>
    <div className="map-toolbar"><Segmented value={group} onChange={setGroup} options={zoneOptions} />{editMode && onPositionsChange && <span className="map-arrange-hint">Перетащите карточку на нужное место</span>}<div className="legend"><span><i className="legend-dot online" /> Свободен</span><span><i className="legend-dot busy" /> Занят</span><span><i className="legend-dot offline" /> Не в сети</span><span><i className="legend-dot maintenance" /> Сервис</span></div></div>
    {positionError && <div className="form-error map-error" role="alert">{positionError}</div>}
    <div className="map-stage">
      <aside className="map-legend-panel"><div className="map-panel-title"><strong>Состояние мест</strong><span>{pcs.length}</span></div><div className="map-legend-item"><i className="legend-square online" /><span>Свободны</span><b>{pcs.filter((pc) => pc.status === "online").length}</b></div><div className="map-legend-item"><i className="legend-square busy" /><span>Активная сессия</span><b>{pcs.filter((pc) => pc.status === "busy").length}</b></div><div className="map-legend-item"><i className="legend-square offline" /><span>Нет связи</span><b>{pcs.filter((pc) => pc.status === "offline").length}</b></div><div className="map-legend-item"><i className="legend-square maintenance" /><span>Сервис</span><b>{pcs.filter((pc) => pc.status === "maintenance").length}</b></div><div className="map-panel-zone">{group === "Все зоны" ? "Все зоны" : group}</div></aside>
      <div className="map-canvas" aria-label="Карта игровых мест">
        {pcs.length ? Array.from({ length: slotCount }, (_, slot) => {
          const placed = slotByIndex.get(slot);
          const showContext = placed && contextPcId === placed.pc.id && !editMode;
          return <div className={`map-slot ${placed ? "occupied" : ""} ${dropTargetSlot === slot ? "drop-target" : ""}`} key={slot} aria-label={`Место ${slot + 1}${placed ? `: ${placed.pc.name}` : ": свободно"}`} onDragOver={(event) => { if (editMode && onPositionsChange) { event.preventDefault(); event.dataTransfer.dropEffect = "move"; setDropTargetSlot(slot); } }} onDragLeave={() => setDropTargetSlot((current) => current === slot ? null : current)} onDrop={(event) => void handleDrop(event, slot)}>
            {!placed && <span className="map-slot-number">#{slot + 1}</span>}
            {placed && <>
              <button className={`map-seat ${placed.pc.status} ${draggedWorkstationId === placed.pc.id ? "is-dragging" : ""}`} draggable={editMode && Boolean(onPositionsChange)} onMouseEnter={(event) => showSessionTooltip(placed.pc, event.currentTarget)} onMouseLeave={() => setSessionTooltip(null)} onFocus={(event) => showSessionTooltip(placed.pc, event.currentTarget)} onBlur={() => setSessionTooltip(null)} onDragStart={(event) => { if (!editMode || !onPositionsChange) return; justDraggedRef.current = true; setPositionError(null); setDraggedWorkstationId(placed.pc.id); event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", placed.pc.id); }} onDragEnd={() => { setDraggedWorkstationId(null); setDropTargetSlot(null); window.setTimeout(() => { justDraggedRef.current = false; }, 0); }} onClick={(event) => { event.stopPropagation(); if (justDraggedRef.current) { justDraggedRef.current = false; return; } if (editMode) onEditPc(placed.pc); else if (placed.pc.status === "online") onSalePc(placed.pc); else onPc(placed.pc); }} onContextMenu={(event) => { event.preventDefault(); setContextPcId(placed.pc.id); }} aria-describedby={placed.pc.sessionSnapshot && placed.pc.status === "busy" ? `session-tooltip-${placed.pc.id}` : undefined} aria-label={`${placed.pc.name}: ${statusMeta[placed.pc.status].label}`}>
                <div className="map-seat-top"><span>{placed.pc.name}</span><span className="map-seat-state">{placed.pc.status === "busy" ? <Wifi size={12} /> : placed.pc.status === "maintenance" ? <Settings size={12} /> : placed.pc.status === "offline" ? <X size={12} /> : <Check size={12} />}</span></div>
                <div className="map-seat-screen"><Computer size={20} /></div>
                <div className="map-seat-bottom"><small>{placed.pc.client || statusMeta[placed.pc.status].label}</small>{placed.pc.tariff && <small>{placed.pc.tariff}</small>}{placed.pc.session && <small><Clock3 size={10} /> {placed.pc.session}</small>}{placed.pc.status === "offline" && placed.pc.lastSeen && <small>HB: {placed.pc.lastSeen}</small>}</div>
              </button>
              <button type="button" className="map-seat-manage" title="Открыть карточку места" aria-label={`Открыть карточку ${placed.pc.name}`} onClick={(event) => { event.stopPropagation(); onPc(placed.pc); }}><PanelRightClose size={12} /></button>
              {showContext && <div className="map-context-menu" role="menu" onClick={(event) => event.stopPropagation()}><strong>{placed.pc.name}</strong><button role="menuitem" disabled={placed.pc.status !== "online"} onClick={() => { setContextPcId(null); onSalePc(placed.pc); }}><Receipt size={13} /> Оформить продажу</button><button role="menuitem" onClick={() => { setContextPcId(null); onPc(placed.pc); }}><PanelRightClose size={13} /> Открыть карточку</button><button role="menuitem" disabled={placed.pc.status !== "online"} onClick={() => { setContextPcId(null); onBookPc(placed.pc.id); }}><CalendarDays size={13} /> Забронировать</button><button role="menuitem" onClick={() => { setContextPcId(null); onEditPc(placed.pc); }}><Settings size={13} /> Настройки места</button></div>}
            </>}
          </div>;
