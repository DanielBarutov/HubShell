...
- [ ] регистрация выполняется по нику, телефону и паролю;
- [ ] вход работает по нику/телефону и паролю;
- [ ] после сброса оператором вход с пустым паролем открывает только форму
  нового пароля с повтором; до сохранения нового пароля рабочие действия
  недоступны;
...
- [ ] операторская гостевая сессия, пришедшая через heartbeat, открывает
  компактный виджет; старый access-gate не остаётся поверх рабочего стола;
- [ ] вход зарегистрированного клиента после регистрации проходит server entry
  decision для фактического `workstation_id`, а не для `device_id`;
...
## 6. Продажи при активной сессии и перевод

Товар можно оформить на месте со статусом `busy`: активная игровая сессия не
мешает продаже товара. Новое игровое время на занятом месте не запускается —
удалите тариф из заказа и оставьте товары. Две разные позиции оформляются как
две идемпотентные backend-продажи; если вторая операция упала, проверьте первую
в истории продаж перед повтором.

Для ручного перевода выберите в checkout способ `Перевод`. Кассовая смена для
него не нужна: backend сохраняет `payment_method=transfer` и подтверждает
операцию без движения cash ledger. Миграция `20260905_0049` должна быть применена
до проверки этого сценария.

## 7. Сеть, heartbeat и restart
