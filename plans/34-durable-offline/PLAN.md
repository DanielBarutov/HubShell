...
5. [ ] Реализовать reconnect batch, partial result, retry/backoff и manual
   review; journal очищать только по подтверждённым sequence.
6. [ ] Подключить operator stale read model и WinUI offline/reconnecting UI.
7. [ ] Добавить crash/restart, disk-full, clock-skew, duplicate и out-of-order
   tests.
...
Не доказаны native crash/disk-full/clock-skew сценарии, operator stale read
model и production security inspection. Требуется уточнить server-authorized
offline local limit и завершить reconnect partial-result/cleanup flow; до этого
offline остаётся source/unit-level, а не release-ready.
