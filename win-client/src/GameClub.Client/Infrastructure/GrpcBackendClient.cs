...
public sealed class GrpcBackendClient : IBackendClient
{
        ...
        return session with
        {
            ...
            ActiveTariff = response.ActiveTariff is null || response.ActiveTariff.CalculateSize() == 0
                ...
                : new SessionTariffSnapshot(
                    ...
                    response.ActiveTariff.PricePerMinuteCents,
                    response.ActiveTariff.FreeMinutes),
    ...
    private static ClientPortalSnapshot ToPortalSnapshot(Clients.ClientPortalSnapshot source) =>
        new(
            ...
            source.Entitlements.Select(entitlement => new ClientPortalEntitlement(
                ...
                entitlement.ActivatedAt is null ? null : ToIsoTimestamp(entitlement.ActivatedAt),
                entitlement.TimeRestricted,
                entitlement.TimeRestricted ? entitlement.SaleWindowStartMinute : null,
                entitlement.TimeRestricted ? entitlement.SaleWindowEndMinute : null,
                entitlement.TimeRestricted ? entitlement.UsageWindowStartMinute : null,
                entitlement.TimeRestricted ? entitlement.UsageWindowEndMinute : null,
                string.IsNullOrWhiteSpace(entitlement.WindowTimezone) ? null : entitlement.WindowTimezone,
                string.IsNullOrWhiteSpace(entitlement.Audience) ? "all" : entitlement.Audience)).ToArray(),
            source.Tariffs.Select(tariff => new ClientPortalTariff(
                ...
                tariff.PriceCents,
                tariff.TimeRestricted,
                tariff.TimeRestricted ? tariff.SaleWindowStartMinute : null,
                tariff.TimeRestricted ? tariff.SaleWindowEndMinute : null,
                tariff.TimeRestricted ? tariff.UsageWindowStartMinute : null,
                tariff.TimeRestricted ? tariff.UsageWindowEndMinute : null,
                string.IsNullOrWhiteSpace(tariff.WindowTimezone) ? null : tariff.WindowTimezone,
                string.IsNullOrWhiteSpace(tariff.Audience) ? "all" : tariff.Audience)).ToArray(),
    ...
    private static SessionPackageSnapshot ToPackageSnapshot(Sessions.PackageSnapshot package) =>
        new(
            ...
            package.Status,
            package.UsageWindowStartMinute,
            package.UsageWindowEndMinute,
