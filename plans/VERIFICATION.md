...
| Avalonia Windows production host | `GameClub.Client.Windows` composes shared Avalonia UI with DPAPI journal, Windows restart/power, command stream, fullscreen/compact window mode and native tray; `GameClub.Client.Windows.sln` replaces legacy WinUI as Windows entry | Linux source build succeeds with `0` warnings/errors; self-contained `win-x64` folder-publish creates `GameClub.Client.Windows.exe` (`152064` bytes, PE32+ GUI) and `hostfxr.dll` | Runtime, actual tray/window placement and kiosk policy need a native Windows machine |
